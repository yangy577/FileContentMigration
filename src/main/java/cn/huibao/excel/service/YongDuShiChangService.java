 ...
 * DES:
 */
public class YongDuShiChangService {
     ...
     * 获取 拥堵时长 1-5 上周6 日
     */
    public Map<DayOfWeek, Row> findYongDuShiChang(XSSFWorkbook yongDuShiChangWorkBook) throws Exception {
        XSSFSheet sheet = yongDuShiChangWorkBook.getSheet(CommonUtil.SHEET_NAME);
        ...
        for (; numOfSetResult < 7; ) {
            ...
            if (isFirst) {
                if (DayOfWeek.SUNDAY == currentDay) {
                    ...
                    throw new Exception("拥堵时长.xlsx 中数据不完整(最后一行数据日期是" + currentDay + ")");
                }
            }

            if (result.containsKey(currentDay)) {
                throw new Exception("拥堵时长.xlsx 中,第" + lastNum + "行附近数据有误，日期不连续");
            }
